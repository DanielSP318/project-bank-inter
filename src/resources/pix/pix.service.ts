...
export default class PixService {
    ...
    async request(value: number, user: Partial<User>){
        ...
        const key = await encodeKey(user.id || '', value, register.id)
     ...
     async pay(key:string, user: Partial<User>) {
         ...
         if(!pixTransaction){
             throw new AppError('Chave inválida para o pagamento', 401)
         }
     ...
     async transaction(user: Partial<User>) {
         ...
         const pixPaying = await pixRepository.find({where: {payingUser: user.id, status: 'close'}, relations: ['reqquestingUser']})
